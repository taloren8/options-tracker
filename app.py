...
⚠️ חשוב מאוד: המחיר הנוכחי של המניה הוזן ידנית על ידי המשתמש ועומד על ${current_price if current_price > 0 else "לא סופק"}.
השתמש אך ורק במחיר זה בניתוח שלך. אל תחפש ואל תציין מחיר אחר בשום אופן.

טרייד: {ticker} {option_type} ${strike} | פקיעה {expiry} ({dte} ימים) | פרמיה ${premium} x{contracts}
מחיר נוכחי (סופי): ${current_price if current_price > 0 else "לא סופק"}
{otm_line}
SL: ${sl} | TP1: ${tp1} | TP2: ${tp2} | נקודת איזון: ~${breakeven:.2f}
קטליסט שצוין: {catalyst or 'לא צוין'}

**1. ניתוח טכני** (3-4 שורות)
בסס את הניתוח על המחיר ${current_price if current_price > 0 else "?"} שסופק. מגמה, מומנטום, תמיכה/התנגדות קרובות.
האם המניה קרובה לסטרייק ${strike}? כמה אחוז צריכה לזוז?

**2. קטליסטים וסקטור** (3-4 שורות)
א) מה הסקטור של {ticker}? יש קטליסט רוחבי על הסקטור הזה כרגע (רגולציה, ריבית, AI, אנרגיה וכו')?
ב) Earnings קרובים על {ticker} או על מניות גדולות באותו סקטור שיכולות להשפיע?
ג) חדשות אחרונות על {ticker} ספציפית — משהו שיזיז אותה תוך {dte} ימים?
